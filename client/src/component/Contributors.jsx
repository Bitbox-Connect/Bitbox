...
// const IssueCard = ({ title, number, html_url, user, state, mode }) => (
//   <motion.div
//     whileHover={{ y: -5 }}
//     transition={{ type: "spring", stiffness: 300 }}
//     className={`rounded-lg overflow-hidden border transition-all duration-300 ${
...
//         </p>
//         <div
//           className={`mt-4 rounded-full py-2 px-3 inline-block ${
//             mode === "dark" ? "bg-white/10" : "bg-blue-100"
//           }`}
//         >
//           <span
...
const IssueCard = ({ title, number, html_url, user, state, mode }) => {
  ...
  // Hover styles based on the `isHovered` state
  const hoverStyles = isHovered
    ? { boxShadow: "1px 1px 12px 4px blue", transform: "translateY(-5px)" }
    : {};
  ...
  return (
    <div
      className={`rounded-lg overflow-hidden border-2 transition-all duration-300 ${
        mode === "dark"
          ? "bg-black/30 backdrop-blur-md border-white/10"
          : "bg-white backdrop-blur-sm  border-gray-600"
      }`}
      style={{ minHeight: "350px", ...hoverStyles }} // Add the hover styles directly to the div
...
const StatCard = ({ label, value, icon, mode, onClick }) => (
  <motion.div
    ...
    className={`rounded-lg p-6 flex items-center border transition-all duration-300 cup ${
      mode === "dark"
        ? "bg-black/30 backdrop-blur-md border-white/10"
        : "bg-white backdrop-blur-sm border-gray-300"
    }`}
...
export default function Contributor(props) {
    ...
    const fetchData = async () => {
      try {
        ...
        const issueResponse = await fetch(
          "https://api.github.com/repos/Bitbox-Connect/Bitbox/issues"
        );
  ...
  const currentItems = showIssue
    ? openIssues.slice(indexOfFirstItem, indexOfLastItem)
    : contributors.slice(indexOfFirstItem, indexOfLastItem);

  // Determine total pages
  const totalPages = showIssue
    ? Math.ceil(openIssues.length / itemsPerPage)
      ...
      <section
        className={`relative h-[70vh] flex items-center justify-center text-center ${
          props.mode === "dark"
            ...
            : "bg-gradient-to-br from-white via-blue-200 to-blue-600"
        }`}
      >
        <div
        ...
        <div className="relative z-10 space-y-6 max-w-4xl mx-auto px-4">
          ...
          <div className="border p-1 bg-slate-950 inline-block rounded-full border-[white] shadow-lg">
            <button className="text-white px-2 py-2 text-[12px] font-bold">
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {/* Contributors */}
            <div className="cursor-pointer shadow-lg rounded-sm contributor-card ">
              <StatCard
                mode={props.mode}
                label="Contributors"
            ...
            <div className="cursor-pointer shadow-lg rounded-sm contributor-card">
              {/* Repository Stars */}
              <a
                href="https://github.com/Bitbox-Connect/Bitbox/stargazers"
                target="_blank"
              >
                <StatCard
                  mode={props.mode}
            ...
            <div className="cursor-pointer shadow-lg rounded-sm contributor-card">
              {/* Repository Forks */}
              <a
                href="https://github.com/Bitbox-Connect/Bitbox/network/members"
                target="_blank"
              >
                <StatCard
                  mode={props.mode}
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto ">
          <h2 className="text-3xl font-bold text-center mb-12">
            {showIssue
              ? "Your contribution is valuable see all Issues"
              : "Contributors"}
          </h2>
          ...
          <div className="flex justify-center mt-8">
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`px-4 py-2 mx-1 rounded  text-black ${
                  i + 1 === currentPage
